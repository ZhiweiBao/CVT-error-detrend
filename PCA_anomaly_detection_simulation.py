...
test_data_1_ = dataset_1.iloc[1000:3000,:]
...
test_data_1_ = scaler_1.transform(test_data_1_)
...
fig = plt.figure()
plot_data_1 = smooth(np.append(Q_1, Q_test_1), 97)
#plot_data_2 = movingaverage(np.append(Q_2, Q_test_2), 96)
plot_data_1_ = smooth(np.append(Q_1, Q_test_1_), 97)
plot_X = list(range(1, len(plot_data_1)+1))
ax = fig.add_subplot(111) 
l1, = ax.plot(plot_X, plot_data_1, linestyle = "dotted", color="black", linewidth=1.0)
#l2, = ax.plot(plot_X, plot_data_2)
l1_, = ax.plot(plot_X, plot_data_1_, color="black", linewidth=1.0)

#ax.hlines(Q_1_low, 1, len(plot_X)+1, linestyles = "dashed")
cl1 = ax.hlines(Q_sigma_1, 1, len(plot_X)+1, linestyles = "dashed", color="red")
#ax.hlines(Q_2_low, 1, len(plot_X)+1, linestyles = "dashed")
#ax.hlines(Q_2_up, 1, len(plot_X)+1, linestyles = "dashed")
ax.vlines(1000, -1, 1, linestyles = "dashed", linewidth=1.0)

#cl1 = ax.hlines(Q_sigma_1, 1, len(plot_X)+1, linestyles = "dashed")
#cl2 = ax.hlines(Q_sigma_2, 1, len(plot_X)+1, linestyles = "dotted")

ax.set_xlim(0,3000)
ax.set_ylim(0, 0.5)

ax.legend(handles = [l1_, l1, cl1], labels = ['εΔ = 0', 'εΔ = +0.1%', 'control limit'
], loc = 'best')

ax.set_xlabel("Sample Number")
ax.set_ylabel("Q statistics")
...
C_alpha = 2.33
Q_sigma_1 = theta1 * (C_alpha * h0 * np.sqrt(2*theta2) / theta1 + 1 + theta2 * h0 * (h0-1) / theta1**2) ** (1/h0)


train_data = train_data_1
U, D, Vh = np.linalg.svd(train_data)
Pe = Vh.transpose()[:, 1:]
